...
1. Empty dependency array 讓 interval callback 保留初次 render 的 count。
2. 這是 JavaScript closure 加上 React snapshots，不是 timer 自己快取。
3. 先指出 captured value，再比較不同同步策略的成本。
...
在 lab selector 選 **06-stale-closure / starter**。
