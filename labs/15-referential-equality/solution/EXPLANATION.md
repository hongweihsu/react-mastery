# Solution explanation｜15-referential-equality
...
04 │  * Lab focus:
05 │  * - 內容相同的兩個 object 仍是不同 identity。
06 │  * - Dependency comparison 使用 Object.is。
07 │  * - 很多情況把 object 建在 Effect 內，比 useMemo 更直接。
08 │  */
09 │ import { useEffect, useState } from "react";
10 │ export default function Lab(){const[n,setN]=useState(0);useEffect(()=>{const options={room:"alpha"};console.log("synchronize using",options)},[]);return <section><button onClick={()=>setN(x=>x+1)}>Unrelated render {n}</button><p>No memo needed when construction moves inside Effect.</p></section>}
11 │ 
```
...
內容相同的兩個 object 仍是不同 identity。 Dependency comparison 使用 Object.is。 很多情況把 object 建在 Effect 內，比 useMemo 更直接。
