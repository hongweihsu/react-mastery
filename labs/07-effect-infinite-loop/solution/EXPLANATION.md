# Solution explanation｜07-effect-infinite-loop
...
04 │  * Lab focus:
05 │  * - 每次 render 建立新的 options object reference。
06 │  * - dependency changed → Effect → state update → render 形成 feedback loop。
07 │  * - 正確問題是 Effect 是否必要，而非如何欺騙 dependency array。
08 │  */
09 │ export default function Lab(){const options={mode:"active"};return <section><h2>Mode: {options.mode}</h2><p>No external system existed, so the correct fix removes the Effect and duplicated state.</p></section>}
10 │ 
```
...
每次 render 建立新的 options object reference。 dependency changed → Effect → state update → render 形成 feedback loop。 正確問題是 Effect 是否必要，而非如何欺騙 dependency array。
