...
> 能先問 external system 是什麼，再決定是否需要 Effect。
...
Effect 在 commit 後讓 React state 與 external system 同步；不是『render 後放程式碼的地方』。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- dependencies 描述讀取的 reactive values；cleanup 撤銷前次同步。
...
前往 [Lab 05-effect-lifecycle](../labs/05-effect-lifecycle/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> dependency array 是你想選擇 Effect 何時執行的開關。
...
先問能否在 render/event 中完成；需要 subscription、timer、network、DOM API 才建立 Effect。
