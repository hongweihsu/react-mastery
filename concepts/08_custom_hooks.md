# 08｜Custom Hooks 與 Stateful Behaviour
...
> 能辨識 custom Hook 抽取的是 stateful behaviour，而不是共用 state instance。
...
Custom Hook 抽取可重用 stateful behaviour；每次呼叫仍擁有獨立 state。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 設計小而具 domain 意義的 API，管理 dependency 與 cleanup。
...
前往 [Lab 10-custom-hook](../labs/10-custom-hook/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
先等重複需求出現；過度 generic hook 會把 caller 迫入 options maze。
