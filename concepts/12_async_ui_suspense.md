# 12｜Async UI、Transitions 與 Suspense
...
> 能分清 urgent/non-urgent update、pending、request cancellation 與 framework data integration。
...
Urgent input 與 non-urgent result update 可有不同 priority；Suspense boundary 協調可暫停的 subtree。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 核心 `startTransition`/`useTransition` 可直接教；data Suspense 的 production integration 常由 framework/cache 提供。
...
前往 [Lab 18-async-ui](../labs/18-async-ui/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> 把 fetch Promise 丟進 Suspense 就是完整 data strategy。
...
明確區分 request error、render error、loading state、transition pending 與 framework responsibility。
