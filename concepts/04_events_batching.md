# 04｜Events、Batching 與 Update Queue
...
> 能用 update queue 解釋 value update 與 updater function 的差別。
...
同一輪更新會被批次處理（Batching）；value update 使用該 render snapshot，updater function 依序套用 queue。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 比較三次 setCount(count+1) 與三次 setCount(c=>c+1)。
...
前往 [Lab 03-batching](../labs/03-batching/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
先描述 queue semantics，再談減少 commits 的效益；updater 適合依前值更新。
