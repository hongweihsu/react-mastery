...
> 能說明 state snapshot、setter queue，以及 state 為何由 tree position 保存。
...
每次 render 取得一份狀態快照（State Snapshot）；setter 排入更新，不改變當前 closure 的值。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 追蹤 render queue、preservation、reset 與 state placement。
...
前往 [Lab 02-state-snapshot](../labs/02-state-snapshot/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
State 最小化並 colocate；不要用同步心智模型讀 setter 後的舊 binding。
