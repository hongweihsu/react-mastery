# 15｜React Architecture 與 State Ownership
...
> 能把 state 放到最近 owner，並以 change boundary 解釋 component 拆分。
...
架構是 ownership、dependency direction 與 change boundary 的結果，不是固定 folder slogan。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- Colocate state、區分 UI/client/server state、避免 giant components 與 premature abstraction。
...
前往 [Lab 20-architecture-refactor](../labs/20-architecture-refactor/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
Feature-specific duplication 有時比錯誤 shared abstraction 更便宜；用 change patterns 決定。
