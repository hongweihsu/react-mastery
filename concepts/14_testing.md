# 14｜Testing User-Observable Behaviour
...
> 能從 user action → visible behaviour 寫測試，而非查 component internals。
...
測試從 user action 到 visible outcome，使用 semantic queries，而非 component internals。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 以 role/name/label 查詢，處理 async UI，辨識 mock 與 brittle test trade-off。
...
前往 [Lab 19-testing-behavior](../labs/19-testing-behavior/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
Accessible query 同時壓力測試語意；implementation refactor 不應破壞 behavioural test。
