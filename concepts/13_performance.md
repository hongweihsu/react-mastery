# 13｜Performance 與 Referential Equality
...
> 能先量測，再用 referential equality 解釋 memoization 是否生效。
...
每次 component execution 可建立新 object/function；memo 比較 props 時 reference identity 會影響 bail-out。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- Observe → Measure → Hypothesize → Change → Measure again。
...
前往 [Lab 16-performance-profiling](../labs/16-performance-profiling/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> 每個 re-render 都是 performance bug；useMemo 永遠更快。
...
Memoization 有 comparison、memory、dependency 與理解成本；以 Profiler evidence 決定。
