...
> 能分開 reducer transition 與 Context delivery，並說明 provider update surface。
...
Reducer 把 state transition 集中；Context 把 value 提供給 subtree，但 provider value 改變會影響 consumers。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 分離 state/dispatch contexts，或用 composition 縮小 broadcast surface。
...
前往 [Lab 13-context-rerender](../labs/13-context-rerender/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
Context 解決傳遞，不自動解決頻率、selector、ownership 或 server-state cache。
