# 05｜Derived State 與 Single Source of Truth
...
> 能找出 source of truth，刪除可由現有資料直接計算的 duplicated state。
...
能由目前 props/state 計算的值通常不需要另一份 state。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 刪除 duplicated state 與同步 Effect，讓 render 純粹推導。
...
前往 [Lab 04-derived-state](../labs/04-derived-state/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
不要用 Effect 修補資料模型；先問 source of truth 在哪裡。
