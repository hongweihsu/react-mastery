...
> 能用 UI 是否需要重畫，判斷值應放 state 或 ref。
...
Ref 保存跨 render 的 mutable cell；改 `.current` 不要求 React 重新 render。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 以『改變是否必須顯示在 UI』選 state 或 ref；DOM ref 是 imperative escape hatch。
...
前往 [Lab 09-ref-vs-state](../labs/09-ref-vs-state/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> useRef 是不會觸發 render 的 useState。
...
Ref 不參與 declarative data flow；避免用它隱藏應顯示的 state。
