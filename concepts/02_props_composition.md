...
> 能說明 props 是某次 render 的 inputs，並比較 data props 與 children composition。
...
Props 是某次 render 的唯讀輸入；composition 把可變部分交給 caller。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 避免 boolean-prop explosion，辨識 reusable 與 feature-specific boundary。
...
前往 [Lab 17-controlled-uncontrolled](../labs/17-controlled-uncontrolled/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> props drilling 一定要用 Context 解決。
...
先考慮 composition、state colocation，再評估 Context 的更新面。
