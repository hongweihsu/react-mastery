# 09｜Identity、Reconciliation 與 Keys
...
> 能用 position/type/key 預測 state preserve/reset。
...
React 以 tree position、element type 與 key 判斷 identity，從而 preserve 或 reset state。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- Key 不只是效能提示，而是 sibling identity。
...
前往 [Lab 11-component-identity](../labs/11-component-identity/README.md)。依 README 先執行 starter，記錄 prediction 與 observation，再比較 solution。
...
> 同一個 component function 永遠共享同一份 state。
...
用 domain identity；index key 在 reorder/delete 時可能把 local state 配錯資料。
