...
> 能指出 ownership、variation points 與 accessibility semantics。
...
API 應表達穩定 variation points，讓 ownership、semantics 與 accessibility 清楚。

## Learning objectives

- 能在執行前預測哪些 components execute、是否 commit、DOM 是否 mutation。
- 比較 controlled/uncontrolled、compound/headless/composition patterns。
...
> Reusable component 的 props 越多越彈性。
...
Boolean explosion 是變化軸不清的訊號；抽象應跟隨已知需求。
